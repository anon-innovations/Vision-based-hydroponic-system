...
while True :
    ...
    adc3 = ads1115.readVoltage(0)
        #Convert voltage to EC with temperature compensation
    EC = ec.readEC(adc2['r'],temperature)
    PH = ph.readPH(adc3['r'],temperature)
    print ("Temperature:%.1f ^C EC:%.2f ms/cm PH:%.2f "%(temperature,EC,PH))
    DataJson = open("/var/www/html/web/JsonECPH.json","w")
    DataJson.write('{"Temperature":%.1f, "EC":%.2f, "PH":%.2f}' % (temperature,EC,PH))
    DataJson.close()
    #sleep(5)
